#!/usr/bin/env python3
"""12-tile Aurora XPU pool manager for async EOS (E–V) tasks.

Pulls relaxed structures from a cell_opt pool directory
(``tasks/<id>/relaxed.extxyz``), runs ``eos_one.py`` on free XPU tiles,
...
Typical use (PBS, one Aurora node)::
  ...
  python xpu_tile_pool.py \\
    ...
    --out-dir      .../workflow_out/eos_mc_sqs_20260730_032035

Does **not** submit PBS. Use ``--dry-run`` to print the launch plan.

Resume: tasks with ``result.json`` where all EOS scheme fits are ``ok`` are
...
from eos_fit import fits_all_ok  # noqa: E402
...
def reap_slot(slot: Slot, tasks: dict[str, Task]) -> bool:
    ...
    if rc == 0 and is_complete(Path(task.out_dir)):
        ...
        if rc == 0:
            detail += " (missing/invalid result.json or incomplete EOS fits)"
        task.error = detail
    slot.proc = None
    slot.task_id = None
    return True
...
def merge_eos_results(tasks: dict[str, Task], out_root: Path) -> Path | None:
    ...
    for r in rows:
        entry: dict[str, Any] = {
            ...
            "n_fits_ok": r.get("n_fits_ok"),
            "n_fits": r.get("n_fits"),
            "wall_s": r.get("wall_s"),
            "B_GPa_by_scheme": {},
            "V0_A3_by_scheme": {},
        }
        for name, fit in (r.get("fits") or {}).items():
            if fit.get("ok"):
                entry["B_GPa_by_scheme"][name] = fit.get("B_GPa")
                entry["V0_A3_by_scheme"][name] = fit.get("V0_A3")
        summary.append(entry)
    atomic_write_json(out_root / "eos_summary.json", summary)
    return out


def main() -> None:
    ap = argparse.ArgumentParser(
        description="Async 12-tile XPU pool manager for EOS",
    ...
    if not EOS_ONE.is_file():
        raise SystemExit(f"missing worker: {EOS_ONE}")
    ...
    volumes_scale = [float(s) for s in args.volumes_scale]
    ...
    t0 = time.time()
    print(
        f"eos_pool: {len(tasks)} tasks, {len(queue)} to run, "
        f"{args.n_tiles} tiles, scales={volumes_scale}, out={out_root}"
    )
    ...
    merged = merge_eos_results(tasks, out_root)
    if merged:
        print(f"wrote {merged} and eos_summary.json")
